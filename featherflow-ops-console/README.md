...
其中页面上的 `workflowName` 直接读取 `workflow_instance.workflow_name`，`bizKey` 直接读取 `workflow_instance.biz_key`，工作流启动节点展示 `workflow_instance.start_node`，每个 activity 尝试的执行节点展示 `activity_instance.executed_node`。

这个项目的目标是帮助运维或研发同学快速查看工作流执行状态、失败步骤、上下文快照，以及通过页面直接发起常见运维操作。

## 文档关系

- 框架整体原理、状态机、DB 锁、幂等、启动恢复和接入方式请先看根目录 [`README.md`](../README.md)。
- 本文档只聚焦 Ops Console：页面入口、健康检查、部署方式、运维动作语义和运行配置。
- 如果只想打开页面排查 workflow，优先阅读“页面入口”和“运维动作语义”。

## 特性

- 工作流列表页，支持按 `workflowId`、`bizId`、`bizKey`、`status`、`workflowName`、创建时间、更新时间筛选
- 工作流详情页，支持查看 workflow 基本信息、activity 时间线、步骤 `input/output`、失败信息
- 工作流详情页支持压缩 Activity 执行链路，用“糖葫芦串”形式按 activity 聚合展示最新结果、总执行次数、失败次数、重试次数和成功次数
- 操作历史页，支持查看 `workflow_operation` 历史、消费状态和原始 `input`
- 定义步骤接口支持按 `workflowName` 查询当前运维台加载到的 XML/YML 编排步骤和 `desc` 描述
- 表格中的 JSON 字段默认折叠预览，可通过右下角 `+` 打开弹窗查看；合法 JSON 会自动格式化，非 JSON 原样显示，长内容会自动换行
- 页面直接提供运维按钮：
  - `Terminate`
  - `Retry`
  - `Skip Latest Activity`
- `workflow` 列表默认每页 `10` 条，activity 时间线默认每页 `5` 条，均支持分页与页大小切换
- 所有运维动作统一写入 `workflow_operation`；`Terminate` / `Retry` 在页面上直接提交，`Skip Latest Activity` 会要求填写操作原因
...
- `/workflow-definitions/{workflowName}/steps`
  工作流定义步骤接口。返回当前运维台通过 `featherflow.definition-locations` 加载到的步骤列表，包括 `sequence`、`activityName`、`desc`、`handler`、`retryInterval` 和 `maxRetryTimes`。
...
curl -i http://127.0.0.1:8080/workflow-definitions/demoOrderWorkflow/steps
```
...
- `Terminate` / `Retry` 不弹窗，不要求填写 `operator` 或 `reason`，会直接写入 `workflow_operation`。
- `Skip Latest Activity` 会弹窗要求填写 `operator` 和 `reason`，这些信息会被编码进 `workflow_operation.input` JSON 中。
- 从 Ops Console 发起的所有命令都会在 `workflow_operation.input` 中写入 `source=FEATHERFLOW_OPS_CONSOLE`，便于和外部系统写入的命令区分。
- 操作提交成功后页面会显示右上角成功提示，并在数秒后自动淡出。

`Skip Latest Activity` 默认由引擎跳过当前最新 activity，不再要求页面显式传入 `activityId`。
...
java -jar featherflow-ops-console/target/featherflow-ops-console-0.0.3-SNAPSHOT.jar
```

如果需要修改端口：

```bash
java -jar featherflow-ops-console/target/featherflow-ops-console-0.0.3-SNAPSHOT.jar --server.port=8081
```
