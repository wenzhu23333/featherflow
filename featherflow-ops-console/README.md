...
- 操作历史页，支持查看 `workflow_operation` 历史及其中的 `operator`、`reason`、`activityId`
- 页面直接提供运维按钮：
  - `Terminate`
  - `Retry`
...
对于 `Skip Latest Activity`，还会自动带上当前最新 activity 的 `activityId`。

这些信息会被编码进 `workflow_operation.input` JSON 中，便于后续引擎消费和页面展示。
