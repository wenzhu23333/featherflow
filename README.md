...
- 支持单文件单 workflow、单文件多 workflow，以及多文件混合加载。
- 持久化 `workflow_instance`、`activity_instance` 和 `workflow_operation` 三张核心表。
- `workflow_instance.biz_key` 可选保存被工作流操作的业务对象标识，例如订单号、workerName、资源 ID 或发布单 ID。
- `workflow_instance.workflow_name` 独立保存工作流定义名称，`workflow_instance.start_node` 保存工作流启动节点。
- `activity_instance.executed_node` 保存每次 activity 尝试的实际执行节点。
- `activity_instance` 采用追加写入模式，每次 activity 完成后都会插入一条成功或失败记录，自动重试预算通过同一 `workflow_id + activity_name` 下的 `FAILED` 历史记录数计算。
...
- demo 默认把 `demoOrderWorkflow` 和 `demoFastTrackWorkflow` 放在同一个 YAML 文件里，演示单文件多 workflow 定义。
...
其中 `workflow_instance` 和 `activity_instance` 的关键字段语义是：

- `workflow_id`：工作流运行实例 ID
- `biz_id`：业务侧传入或自动回落的业务标识
- `biz_key`：可选的业务对象标识，不做唯一约束，便于按订单号、资源 ID、workerName 等维度运维检索
- `workflow_name`：工作流定义名称，对应 YAML/XML 里的工作流 `name`
- `start_node`：创建并首次调度该 workflow 的节点标识
- `executed_node`：每条 activity 尝试记录对应的实际执行节点标识
- `activity_instance`：一次 activity 完整执行结束后落一条记录，成功和失败都会保留，重试预算通过失败历史统计得到
...
- XML 工作流定义使用 JDK 内置 DOM 解析，不需要额外引入 `jackson-dataformat-xml`。
- 不同文件里的 workflow `name` 必须唯一；如果重名，应用会在启动时直接失败，避免定义被静默覆盖。
...
<workflow name="sampleOrderWorkflow">
  <activity name="createOrder" handler="createOrderHandler" desc="创建订单 / Create order" retryInterval="PT5S" maxRetryTimes="2"/>
...
- `desc`：可选的步骤说明，支持中英文描述；YAML 使用 `desc` 字段，XML 支持 `desc="..."` 或 `<desc>...</desc>` 子节点。
- `retryInterval`：失败后的重试间隔，使用 `Duration` 格式。
- `maxRetryTimes`：最大重试次数，超过后进入人工处理状态。
- 顶层既支持 `workflow:`，也支持 `workflows:`；使用 `workflows:` 时，单个文件内的多个 workflow `name` 也必须保持唯一。

## 定义步骤查询

Starter 会自动提供 `WorkflowDefinitionQueryService`，可按 workflow 定义名称查询完整步骤清单：
...
返回字段包括 `workflowName`、`activityName`、`desc`、`handler`、`retryInterval`、`maxRetryTimes` 和 `sequence`。Ops Console 会加载同一套 `definition-locations`，并提供：
...
- `startWorkflow(definitionName, bizId, bizKey, input)`：新增四参重载，`bizKey` 表示被操作的业务对象标识；老三参接口继续保留，并默认写入 `null`。
...
- 外部 `START` operation 的 `input` 如果包含 `bizKey`，daemon 消费时会同步写入 `workflow_instance.biz_key`。

## 压缩 Activity 执行链路

原始 `activity_instance` 时间线仍然完整保留。运维查询侧会按 `workflow_id + activity_name` 聚合，返回每个步骤最新一次执行结果，同时计算：

- `totalAttempts`：该步骤总执行次数。
- `failedTimes`：失败次数。
- `retryTimes`：`max(totalAttempts - 1, 0)`。
- `successfulTimes`：成功次数。

这个压缩视图适合前端用“糖葫芦串”形式一眼看完整工作流状态；需要排查细节时仍然回看原始 activity 时间线。
